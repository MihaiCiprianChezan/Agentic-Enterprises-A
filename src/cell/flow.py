...
def _actor_of(role: object, role_name: str) -> ActorRef:
    ...
    return (
        actor if isinstance(actor, ActorRef) else ActorRef(role=role_name, version="unattributed")
    )
...
def run_flow(
    ...
    *,
    ...
    with tracer.span(
        "specify", _actor_of(director, "Director"), "decision", input_digest=digest(ticket)
    ) as span:
    ...
    with tracer.span(
        "decompose", orchestrator_actor, "decision", input_digest=digest(goal)
    ) as span:
...
def _produce_and_verify(
    ...
    while True:
        with tracer.span(
            "execute", _actor_of(executor, "Executor"), "tool_call", input_digest=digest(item.id)
        ) as span:
        ...
        with tracer.span(
            "verify",
            _actor_of(verifier, "Verifier"),
            "verification",
            input_digest=digest(output.id),
        ) as span:
