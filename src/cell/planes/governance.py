...
class RuleSetGovernance:
    ...
    def decide(self, action: Any, actor: Any) -> GovernanceDecision:
        ...
        if human and _LEVEL_RANK[level] <= _LEVEL_RANK["L0"]:
            return GovernanceDecision(
                ...
                "Office authority confers nothing (Art. 9)",
                novel,
            )
        ...
        if action_class in _BOUNDARY_CLASSES:
            return GovernanceDecision(
                ...
                "on the cell's own authority (Art. 2.2)",
                novel,
            )
        ...
        if level == "L0":
            return GovernanceDecision(
                ...
                "requires a static breakpoint and a human (Art. 5.2)",
                novel,
            )
        ...
        return GovernanceDecision(
            ...
            RULE_CLAUSES["R1"],
            level,
            f"{level} action {action_class} permitted ({autonomy}) per Art. 4",
            novel,
        )
    ...
    def evaluate_and_log(
        self, action: Any, actor: Any, store: Any, flow_id: str
    ) -> GovernanceDecision:
