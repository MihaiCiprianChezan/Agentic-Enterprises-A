...
class RealExecutor:
    def __init__(
        self, runner: Runner, checkout_dir: str, branch: str, *, actor: ActorRef = EXECUTOR_ACTOR
    ) -> None:
        self.runner = runner
    ...
    def execute(self, item: WorkItem) -> Output:
        ...
        if result.returncode != 0:
            raise ExecutorError(
                f"agent run failed (rc={result.returncode}): {result.stderr[-500:]}"
            )
        ...
        _git(
            [
                "-c",
                "user.email=cell@local",
                "-c",
                "user.name=cell",
                "commit",
                "-q",
                "-m",
                f"cell: {item.description[:60]}",
            ],
            self.checkout_dir,
        )
        ...
        return Output(
            ...
            artifact_ref=f"branch:{self.branch}@{sha}",
