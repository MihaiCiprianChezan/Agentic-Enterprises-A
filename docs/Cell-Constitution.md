...
- **3.4 Deferred roles.** Optimizer and Auditor are not instantiated until their precondition exists (capability/cost spread; multiple running versions). Their seams are left open, unused. **Forward note:** when the Auditor is switched on, this constitution must first be amended to add the governed content the model requires for it — the suspension threshold (reserved for *danger*, not ordinary regression) and a defined human-response SLA for any suspended-but-critical version (model §11). Until then the Auditor's only contemplated future role is surfacing proposed amendments (Art. 8.4).
