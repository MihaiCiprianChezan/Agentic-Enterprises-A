...
- **3.4 Deferred roles.** A role is instantiated only once its precondition exists (capability/cost spread; multiple running versions). The **Optimizer is now instantiated** (M8): its precondition — attributed cost and capability/cost spread across implementers — holds. It is bound by the risk→capability floor declared as constitutional content (`CAPABILITY_FLOOR`, Art. 4 / governance registry): it may minimize cost only *beneath* a floor it does not set, and may never route a task below it. The **Auditor remains deferred**; its seam is left open, unused. **Forward note:** when the Auditor is switched on, this constitution must first be amended to add the governed content the model requires for it — the suspension threshold (reserved for *danger*, not ordinary regression) and a defined human-response SLA for any suspended-but-critical version (model §11). Until then the Auditor's only contemplated future role is surfacing proposed amendments (Art. 8.4).
