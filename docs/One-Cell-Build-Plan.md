...
| Auditor | **9b built** | M9 | Preconditions done (versions enabler + Article 11 amendment). **9b (rate + report) is built** (`cell/auditor.py`): rates each version from field activity into a per-role fitness leaderboard, flags regressions/danger as audit records, faithful to Art. 11 (danger = safety breach; collapse = alert-only). **Only 9c remains** — the suspend-and-escalate breaker that acts on `dangerous` ratings. |
