...
| Auditor | **Preconditions met** | M9 | YAGNI (§11): nothing to compare until you run multiple versions. **Both preconditions now done** — the versions enabler (`cell/versions.py`) makes role versions first-class/statused/scored, and the **constitutional amendment is enacted** (Article 11 + `SUSPENSION_POLICY`, ratified 28.06.2026). The Auditor itself — 9b (rate + report) then 9c (the suspend-and-escalate breaker) — lands next. |
