...
| **Authority scope** | **One governed action: suspend (M9c).** May suspend a version it rates `dangerous` (`registry.set_status`, which the Optimizer respects) — **unilateral, for safety** — but may **not** reinstate (un-pause is the Steward's/a human's), modify/retune a version (Steward), dismiss/retire one (Board), or make any business decision. **Danger is governed**, not improvised: only a **safety breach** (Art. 11) — never a mere quality regression — is suspended. |
| **Breaker bounds (M9c)** | `Auditor.enforce(now)` suspends within `SUSPENSION_POLICY`: **rate-limited** (excess dangerous versions are *escalated, not auto-suspended* — no cascade); a **critical** suspension (no other active version of the role) opens the **24h SLA**; an expired SLA still suspended escalates to **break-glass**. Every act is logged on `__audit__` (+ the registry status change). |
