...
def test_selects_cheapest_implementer_that_clears_the_floor():
    ...
    chosen = CostAwareOptimizer().select(
        _item("CLASS_OWN_WRITE"), cands, {"strong": 10.0, "light": 2.0, "std": 5.0}
    )
...
def test_high_blast_floor_excludes_a_cheaper_low_tier_implementer():
    ...
    chosen = CostAwareOptimizer().select(
        _item("CLASS_HIGH_BLAST"), cands, {"strong": 10.0, "light": 1.0}
    )
    assert (
        chosen.id == "strong"
    )  # L0 floor=3: light excluded though cheapest — never below the floor


def test_no_capable_implementer_raises():
    with pytest.raises(NoCapableImplementer):
        CostAwareOptimizer().select(
            _item("CLASS_HIGH_BLAST"), [_imp("light", 1, 1.0)], {"light": 1.0}
        )
...
def _routed_cell(store, *, boom_cheap=False):
    imps = [
        Implementer("cheap", 1, _FakeImpl("cheap", boom=boom_cheap), nominal_cost=1.0),
...
def test_no_routing_with_fewer_than_two_implementers():
    ...
    assert not [
        e for e in store.read("f") if e.payload.get("stage") == "route"
    ]  # uniform → no router
...
def test_execute_is_attributed_to_the_routed_implementer_not_the_executor_actor():
    store = InMemoryEventStore()
    imps = [
        Implementer("light", 1, _MistaggedImpl(), nominal_cost=1.0),
        Implementer("strong", 3, _FakeImpl("strong"), nominal_cost=9.0),
    ]
    Cell.assemble(store=store, optimizer=CostAwareOptimizer(), implementers=imps).submit(
        _ticket(), "f"
    )
    execs = [e for e in store.read("f") if e.payload.get("stage") == "execute"]
    assert execs[0].payload.get("implementer") == "light"  # tagged with the routed id
    assert (
        mean_cost_for(store.all_events(), "light") == 7.0
    )  # attributed despite the actor mismatch
...
def test_resume_with_a_missing_recorded_implementer_falls_back_without_crashing():
    ...
    other = [
        Implementer("other", 1, _FakeImpl("other"), 1.0),
        Implementer("another", 1, _FakeImpl("another"), 2.0),
    ]  # re-assembled WITHOUT "cheap"
    v = Cell.assemble(store=store, optimizer=CostAwareOptimizer(), implementers=other).submit(
        _ticket(), "f"
    )
